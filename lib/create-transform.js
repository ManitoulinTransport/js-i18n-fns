...
function createTransform ({shouldExtract, extractedMessagesFile, functionIdentifier, localesDir, targetLocales}) {
      ...
      transform (buffer) {
        ...
        traverse(ast, {
          CallExpression (path) {
            ...
            const isFirstArgumentStringLiteral = () => path.get('arguments.0').isStringLiteral()
            const isOccurrence = (isCallOfPlainIdentifier() || isCallOfMethod()) && isFirstArgumentStringLiteral()
            if (isOccurrence) {
